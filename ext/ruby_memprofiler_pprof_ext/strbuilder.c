...
void mpp_strbuilder_appendf(struct mpp_strbuilder *str, const char *fmt, ...) {
  ...
  // the null terminator.
  size_t attempted_writesize_wo_nullterm = vsnprintf(str->curr_ptr, max_writesize, fmt, fmtargs);
...
VALUE mpp_strbuilder_to_value(struct mpp_strbuilder *str) { return rb_str_new2(str->original_buf); }
